import argparse
from utils import random_splits, heter_fixed_splits, get_data_load, random_split
...
def RunExp(args, dataset, data, Net):
    ...
    def train(model, optimizer, data):
        ...
        if args.dataset.lower() in ["minesweeper", "tolokers", "questions"]:
            loss = F.binary_cross_entropy_with_logits(logits.squeeze(-1), data["label"][data["train_mask"]].to(torch.float))
        else:
            loss = F.cross_entropy(logits, data["label"][data["train_mask"]])
        loss.backward()
        optimizer.step()
        del logits
        ...
        with torch.no_grad():
            ...
            for mask in [data["train_mask"], data["val_mask"], data["test_mask"]]:
                if args.dataset.lower() in ["minesweeper", "tolokers", "questions"]:
                    ...
                    acc = roc_auc_score(y_true=data["label"][mask].cpu().numpy(), y_score=logits[mask].squeeze(-1).cpu().numpy())
                    loss = F.binary_cross_entropy_with_logits(logits[mask].squeeze(-1), data["label"][mask].to(torch.float))
                else:
                    pred = logits[mask].max(1)[1]
                    acc = pred.eq(data["label"][mask]).sum().item() / mask.sum().item()
                    loss = F.cross_entropy(model(data)[mask], data["label"][mask])
    ...
    if args.dataset.lower() in ['citeseer', 'pubmed', 'cs', 'physics']:
        train_rate = 0.6
        val_rate = 0.2
        percls_trn = int(round(train_rate*len(data["label"])/dataset.num_classes))
        val_lb = int(round(val_rate*len(data["label"])))
        data = random_splits(data, dataset.num_classes, percls_trn, val_lb, args.seed)
    else:
        data = random_split(data, frac_train=0.48, frac_valid=0.32, frac_test=0.20, seed=args.seed)

    model = tmp_net.to(args.device)
