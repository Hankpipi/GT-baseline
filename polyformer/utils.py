...
def random_split(data, frac_train, frac_valid, frac_test, seed):
...
def get_data_load(args):
    ...
    if args.dataset.lower() in ["minesweeper", "tolokers", "questions"]:
        ...
        args.num_classes = dataset.label.max().item() + 1
    args.num_features = node_feat.shape[1]
    ...
    data["list_mat"] = load_base(args.dataset.lower(), args.base, args.K, node_feat, edge_index, None)
