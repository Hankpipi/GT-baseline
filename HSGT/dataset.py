...
def load_arxiv_year_dataset(data_dir, nclass=5):
    ...
    dataset.data = PygNodePropPredDataset(name='ogbn-arxiv', root=f'{data_dir}ogb')
...
def load_ogb_dataset(data_dir, name):
    ...
    dataset.data = ogb_dataset
    dataset.graph = dict()
    dataset.graph['edge_index'] = torch.as_tensor(ogb_dataset.edge_index)
    dataset.graph['node_feat'] = torch.as_tensor(ogb_dataset.x)
    dataset.graph['num_nodes'] = ogb_dataset.num_nodes
    ...
    # dataset.label = torch.as_tensor(ogb_data.y.data[all_idx], dtype=int).reshape(-1, 1)
    dataset.label = torch.as_tensor(ogb_dataset.y.data, dtype=int).reshape(-1, 1) # 99% labels are nan, not available
    # print(f'f1:{dataset.label.shape}')
    # print(f'{ogb_data.num_nodes}')
    def get_idx_split():
        split_idx = {
...
def load_planetoid_dataset(data_dir, name):
    ...
    dataset.graph = {'edge_index': edge_index,
                     ...
                     'edge_feat': None,
                     'num_nodes': num_nodes}
    dataset.label = torch.as_tensor(label, dtype=int).reshape(-1, 1)
    return dataset
